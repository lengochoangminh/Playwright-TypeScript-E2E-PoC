...
test.describe("Tables and Data - Smart Table", () => {
  ...
  test("@Medium - Verify to update the user's age based on their email", async ({
    page,
  }) => {
    const tableAndData = new TableAndData(page);
    await allure.step(
      "Verify to update the user's age based on their email",
      async () => {
  ...
  test("@Low - Verify the filter the users by years old", async ({ page }) => {
    ...
    for (const age of ages) {
      await allure.step(
        `Verify the filter the users have ${age} years old`,
        async () => {
          await allure.step(
            `Filter the users have ${age} years old`,
            async () => {
