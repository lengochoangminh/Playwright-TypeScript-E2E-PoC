...
export class ChartsPage extends BasePage {
    ...
    async assertPieChart() {
        expect(await this.page.locator('ngx-echarts-pie canvas').screenshot()).toMatchSnapshot();
    }

    async assertBarChart() {
        expect(await this.page.locator('ngx-echarts-bar canvas').screenshot()).toMatchSnapshot({ threshold: 0.5 });
    }
}